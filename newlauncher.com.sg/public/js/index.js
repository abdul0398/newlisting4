...
const filterListings = (input) => {
    ...
    const filter_listings = listings.filter(listing => {
        if (project_names_checked.length == 0 && region_checked.length == 0 && location_checked.length == 0) {
            return true;
        }

        const region = listing?.details?.find(detail => detail.title === "Market Segment");
        const location = listing?.details?.find(detail => detail.title === "Postal District");
        ...
        return is_name_found && is_region_found && is_location_found;
    });
